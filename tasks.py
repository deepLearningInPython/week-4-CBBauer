...
# Your code here:
# -----------------------------------------------
token_to_id = {token: key for key, token in enumerate(sorted(set(tokens)))}
id_to_token = {key: token for key, token in enumerate(sorted(set(tokens)))}
...
#     # extract weights
#     W = np.reshape(w[0:9], (3, 3))
#     U = np.reshape(w[9:18], (3, 3))
#     B = np.reshape(w[19:22], (1,3))
    
#     # output list
#     outputs = [np.zeros(len(list_of_sequences))]
    
#     # iteration over sequences
#     for X in enumerate(list_of_sequences):
#         a = np.zeros(3)
